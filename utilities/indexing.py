...
def nearest_index(xp: float, x: NDArray[np.float64]) -> int:
...
def starting_index(xp: float, x: NDArray[np.float64], m: int = 1) -> int:
...
def bubble_sort(x: NDArray[np.float64], y: NDArray[np.float64]

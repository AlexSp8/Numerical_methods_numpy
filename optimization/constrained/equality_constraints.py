...
def augmented_lagrangian(f: Callable[[NDArray[np.float64]], float],
    g_con: Callable[[NDArray[np.float64]], NDArray[np.float64]],
    x0: NDArray[np.float64],
    df: Callable[[Callable[[float], float], float, float, float], float],
    l0_m: NDArray[np.float64] = None, output: bool = False, tol: float = 1e-6,
    k_max: int = 100) -> tuple[NDArray[np.float64], NDArray[np.float64]]:
    """Returns the optimum point and corresponding Lagrange multipliers
    of a multi-variable function under equality constraints using the
    ...
    for k in range(1, k_max+1):
        ...
        # def f_lagrangian(x_c: NDArray[np.float64]) -> float:
        #     v = g_con(x_c)
...
class AugmentedLagrangian:
    ...
    def f_lagrangian(self, x: NDArray[np.float64]) -> float:

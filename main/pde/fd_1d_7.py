...
def u_analytical(t: float, x: np.ndarray[tuple[int, int]]) -> np.ndarray[tuple[int]]:
    ...
    A = 50.0
    C = 50.0
    B = -5.0/np.cosh(1.0) - 50.0*np.tanh(1.0)
    D = 5.0/np.cos(1.0) + 50.0*np.tan(1.0)
    
    u0 = A*np.cosh(xm) + B*np.sinh(xm) + C*np.cos(xm) + D*np.sin(xm)
    u1 = -A*np.cosh(xm) - B*np.sinh(xm) + C*np.cos(xm) + D*np.sin(xm)

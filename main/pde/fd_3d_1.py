...
def bc_z0(t: float, x: np.ndarray[tuple[int]], u: np.ndarray[tuple[int]],
    grad_u: np.ndarray[tuple[int, int]]) -> np.ndarray[tuple[int]]:
    neq = u.shape[0]
    res = np.zeros(neq)
    xm, ym, zm = x[0], x[1], x[2]
...
def bc_zf(t: float, x: np.ndarray[tuple[int]], u: np.ndarray[tuple[int]],
    grad_u: np.ndarray[tuple[int, int]]) -> np.ndarray[tuple[int]]:
    neq = u.shape[0]
    res = np.zeros(neq)
    xm, ym, zm = x[0], x[1], x[2]
...
def f_res(t: float, x: np.ndarray[tuple[int]], u: np.ndarray[tuple[int]],
    ...
    res_u = u_t - (d2udx2 + d2udy2 + d2udz2) - s
    
    return np.array([res_u])
...
def main():
    ...
    bc = [bc_x0, bc_xf, bc_y0, bc_yf, bc_z0, bc_zf]
